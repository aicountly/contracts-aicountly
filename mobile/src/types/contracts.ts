 ...
 *
 * Mirrored verbatim from `web/src/services/apiClient.ts`'s sibling
 * `web/src/types/contracts.ts` — same PHP API, same envelope, so the same
 * shapes apply unchanged on mobile. Keep this file in sync with the web copy
 * by hand (there is no schema to generate either from): a field added
 * server-side needs adding here too, and a field the API stops sending is a
 * compile error at every call site rather than an `undefined` rendered as
 * text on a phone screen.
 *
 * Anything a second screen could plausibly need lives here. Screen-only
 * shapes stay next to the screen.
 */
 ...
 * every field is optional and the chart mapper reads whichever arrived.
 */
 ...
 *
 * Built by the server rather than assembled on-device: a clause hit goes to
 * the library or to a contract's clauses tab depending on where it was found,
 * and duplicating that rule here would put two answers to one question in two
 * repositories. `link_path` is a web SPA route (e.g. `/contracts/42`); the
 * mobile router maps it onto its own Expo Router path rather than using it
 * directly — see src/utils/routeParams.ts.
 */
interface SearchHit {
  id: number
  link_path: string
