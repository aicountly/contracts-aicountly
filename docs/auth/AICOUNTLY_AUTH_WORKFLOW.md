# AICOUNTLY auth workflow (Contracts)

How Contracts signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `contracts.aicountly.com` (or `contracts.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/contracts?returnUrl={origin}/auth/callback`.
...
| Contracts host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `contracts.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `contracts.aicountly.com/api` |
| `contracts.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `contracts.gh.aicountly.com/api` |
...
`contracts` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://contracts.gh.aicountly.com/api/health
...
curl -i -X POST https://contracts.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://contracts.gh.aicountly.com/api/global/login   # expect 404
```
